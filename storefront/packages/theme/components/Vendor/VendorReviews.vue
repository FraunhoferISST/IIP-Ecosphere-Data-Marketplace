...
<div>
  ...
  <data-viewer :loading="load.loading.value" :error="load.error.value">
    <div class="mb-10" v-if="showReviewForm && (!canAddReview.loading.value && !canAddReview.error.value)">
      <vendor-review-form :shop-id="shopId"/>
    </div>
...
export default {
  name: "VendorReviews",
  components: {VendorReviewForm, ReviewsAggregation, ProductReviewForm, DataViewer, SfReview, SfAlert},
  props: {
    shopId: {
      type: String,
      required: true,
  ...
  setup(props) {
    ...
    load.execute({ attributedShopId: props.shopId });
    loadAggregation.execute({ attributedShopId: props.shopId });
    canAddReview.execute(props.shopId).then(result => showReviewForm.value = result).catch(e => console.error(e));
    return {
      showReviewForm,
      reviews,
      reviewsAggregation,
