...
import Vue from "vue";
import { useInquiriesFacets } from "@vue-storefront/iiepmarket";
import _ from "lodash";

export default {
  name: "InquiriesFiltersSidebar",
  components: {
    SfButton,
    SfSidebar,
    SfFilter,
  ...
  setup(props, context) {
    ...
    const { search, result } = useInquiriesFacets();

    const selectedFilters = ref({});

    const facets = computed(() => {
      return (result.value ?? []).map(({ property, values }) => ({
